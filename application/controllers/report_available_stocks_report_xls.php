...
class report_available_stocks_report_xls extends CI_Controller{
    ...
    function print_excel($cmb_item_group){	
        ...
        # MAIN HEADER
        $worksheet->write(0,0,"AVAILABLE STOCKS REPORT",$headerFormat);
        for($i=1;$i<10;$i++) {
        ...
        foreach($data as $row){  
            ...
            if($row->no_of_items < $row->lower_limit)
            {
                $worksheet->write($ctr,6,$row->no_of_items,$row_right_red_font);    
            }
            else
            {
