...
class report_detailed_report_xls extends CI_Controller{
    ...
    function print_excel($txt_date_from,$txt_date_to,$cmb_branch_group){	
        ...
        # FILENAME
        $filename = "detailed_report.xls";
        
        # SEND FILENAME
        $workbook->send($filename);
        
        # ADD WORK SHEET OR WORK SHEET LABEL
        $worksheet = $workbook->addWorksheet('detailed_report');
        ...
        # MAIN HEADER
        $worksheet->write(0,0,"DETAILED REPORT DATE ENTER FROM ".$txt_date_from." TO ".$txt_date_to,$headerFormat);
        for($i=1;$i<11;$i++) {
        ...
        $data = $this->get_report->get_detailed_report($txt_date_from,$txt_date_to,$cmb_branch_group);
