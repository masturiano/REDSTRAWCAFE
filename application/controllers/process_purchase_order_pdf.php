...
class Process_Purchase_Order_Pdf extends CI_Controller {
    ...
    public function index($order_no)
    {
        ...
        $header = $this->get_process_purchase_order->get_header_purchase_order_no_pdf($order_no);
        ...
        $this->pdf->Cell(0,5,'ORDER FORM',0,1,'R');
        $this->pdf->Cell(0,5,'RedStraw Cafe',0,1,'L');
        $this->pdf->Cell(0,5,"The frappe that you can't resist...",0,1,'L');
        
        $this->pdf->SetFont('Arial','',12);
        $this->pdf->Cell(148,6,'ORDER # : ',0,0,'R');
        $this->pdf->Cell(0,6,$header_row->purchase_order_no,0,1,'R');
        $this->pdf->Cell(148,6,'CUSTOMER ID : ',0,0,'R');
        $this->pdf->Cell(0,6,$header_row->branch_no,0,1,'R');
