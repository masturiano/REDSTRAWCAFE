...
class report_cancelled_order_report_xls extends CI_Controller{
    ...
    function print_excel($txt_date_from,$txt_date_to,$cmb_branch_group){	
        ...
        # FILENAME
        $filename = "cancelled_order_report.xls";
        
        # SEND FILENAME
        $workbook->send($filename);
        
        # ADD WORK SHEET OR WORK SHEET LABEL
        $worksheet = $workbook->addWorksheet('cancelled_order_report');
        ...
        # MAIN HEADER
        $worksheet->write(0,0,"CANCELLED ORDER REPORT DATE ENTER FROM ".$txt_date_from." TO ".$txt_date_to,$headerFormat);
        for($i=1;$i<11;$i++) {
        ...
        # HEADER
        $worksheet->write(2,0,"ORDER NO",$headerFormat);
        $worksheet->write(2,1,"BRANCH ID",$headerFormat);
        $worksheet->write(2,2,"BRANCH NAME",$headerFormat);
        $worksheet->write(2,3,"DATE ENTER",$headerFormat);
        $worksheet->write(2,4,"DESCRIPTION",$headerFormat);
        $worksheet->write(2,5,"UNIT PRICE",$headerFormat);
        $worksheet->write(2,6,"BUYER PRICE",$headerFormat);
        $worksheet->write(2,7,"NO OF ITEMS",$headerFormat);
        $worksheet->write(2,8,"ADDED BUYER PRICE",$headerFormat);
        $worksheet->write(2,9,"TOTAL UNIT PRICE",$headerFormat);
        $worksheet->write(2,10,"NET SALES",$headerFormat);
        
        # DATABASE
        $this->load->model('get_report'); 
        
        $data = $this->get_report->get_cancelled_order_report($txt_date_from,$txt_date_to,$cmb_branch_group);
        ...
        foreach($data as $row){  
            ...
            $worksheet->write($ctr,0,' '.$row->purchase_order_no,$row_left);
            $worksheet->write($ctr,1,$row->branch_no,$row_left);
            $worksheet->write($ctr,2,$row->branch_name,$row_left);
            $worksheet->write($ctr,3,date('Y-m-d',strtotime($row->date_enter)),$row_center);
            $worksheet->write($ctr,4,$row->description,$row_left);
            $worksheet->write($ctr,5,$row->unit_price,$row_right_number);
            $worksheet->write($ctr,6,$row->buyer_price,$row_right_number);
            $worksheet->write($ctr,7,$row->input_no_of_items,$row_right);
            $worksheet->write($ctr,8,$row->added_price,$row_right_number);
            $worksheet->write($ctr,9,$row->total_unit_price,$row_right_number);
            $worksheet->write($ctr,10,$row->net_sales,$row_right_number);
        ...
        $data_total_unit_price = $this->get_report->get_total_unit_price_cancelled_order_report($txt_date_from,$txt_date_to,$cmb_branch_group);
        if ($data_total_unit_price->num_rows() > 0){
           $row_total_unit_price = $data_total_unit_price->row();
        }  
        $data_total_buyer_price = $this->get_report->get_total_buyer_price_cancelled_order_report($txt_date_from,$txt_date_to,$cmb_branch_group);
        if ($data_total_buyer_price->num_rows() > 0){
           $row_total_buyer_price = $data_total_buyer_price->row();
        } 
        $data_total_input_no_of_items = $this->get_report->get_total_no_of_items_cancelled_order_report($txt_date_from,$txt_date_to,$cmb_branch_group);
        if ($data_total_input_no_of_items->num_rows() > 0){
           $row_total_input_no_of_items = $data_total_input_no_of_items->row();
        } 
        $data_total_added_buyer_price = $this->get_report->get_total_added_buyer_price_cancelled_order_report($txt_date_from,$txt_date_to,$cmb_branch_group);
        if ($data_total_added_buyer_price->num_rows() > 0){
           $row_total_added_buyer_price = $data_total_added_buyer_price->row();
        } 
        $data_total_total_total_unit_price = $this->get_report->get_total_total_unit_price_cancelled_order_report($txt_date_from,$txt_date_to,$cmb_branch_group);
        if ($data_total_total_total_unit_price->num_rows() > 0){
           $row_total_total_unit_price = $data_total_total_total_unit_price->row();
        } 
        $data_total_net_sales = $this->get_report->get_total_net_sales_cancelled_order_report($txt_date_from,$txt_date_to,$cmb_branch_group);
